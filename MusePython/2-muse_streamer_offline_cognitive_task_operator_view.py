...
TARGET_MAC_ADDRESS = "00:55:DA:B5:61:C3"
...
# Global pool of technical words to prevent repetition and habituation
WORD_POOL = [
...
class OperatorDiagnostics:
    def __init__(self, start_event, exit_event, sys_state, sys_time, base_alpha, task_type):
        ...
        if num_displays == 1:
            os.environ['SDL_VIDEO_WINDOW_POS'] = "20,50"  # Push Operator View to Top-Left
            ...
            if len(data_array) >= SAMPLES_PER_EPOCH:
                ...
                try:
                    ...
                    if c_state == 4 and self.base_alpha.value > 0:
                        # ERD logic: If Alpha does not drop below 85% of baseline during task
...
if __name__ == "__main__":
    ...
    CalibrationProtocol(session_folder, sys_start_event, sys_exit_event, sys_state_val, sys_time_val,
                        sys_base_alpha, task_type).run()
